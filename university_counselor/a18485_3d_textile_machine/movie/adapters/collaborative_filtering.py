...
movies = csv_reader('ml-1m/movies.csv')
print(len(movies), movies[10])
moive_name_dic = {}
for m in movies[1:]:
    if int(m[0]) not in moive_name_dic:
        moive_name_dic[int(m[0])] = m[1]

print(moive_name_dic)
class ItemBasedCF:
            ...
            for user, items in self.train.items():
                ...
                for i in items.keys():
                    ...
                    for j in items.keys():
                        ...
                        if (
                            self.train[str(user)][i]["rate"] > 0.0
                            and self.train[str(user)][j]["rate"] > 0.0
                            and i != j
                        ):
                            count[i][j] += (
                                1
            ...
            for i, related_items in count.items():
                ...
                for j, num in related_items.items():
                    ...
                    itemSim[i][j] = num / math.sqrt(
                        item_eval_by_user_count[i] * item_eval_by_user_count[j]
                    )
    ...
    def recommend(self, user, k=8, nitems=40):
        ...
        for i, rate_time in u_items.items():
            for j, wj in sorted(
                self.items_sim[i].items(), key=lambda x: x[1], reverse=True
            )[0:k]:
                if j in u_items:
                    continue
                ...
                result[j] += (
                    ...
                    * wj
                    * 1
                    / (1 + self.beta * (self.max_data - abs(rate_time["time"])))
                )
...
if __name__ == "__main__":
    ib = ItemBasedCF("ml-1m/ratings.dat")
    result = ib.recommend("1")
    recommend_results = []
    print("user '1' recommend result is {} ".format(result))
    for r in result:
        if len(recommend_results) < 5:
            recommend_results.append(moive_name_dic[int(r)])
            print(moive_name_dic[int(r)], '#'*10)
    write_to_csv('../../recommend.txt', recommend_results)
