...
但是用户注册，登陆、模型介绍博客管理（增删改查）、3d模型web加载展示 都有的，只是没有购买功能

还需要上传obj功能 ，用户添加描述，然后加载obj为3d web模型

 但是我默认给你上传一个 这样做样例，网站首页也是个list 也好看些
...
@app.route("/webgl_loader_obj", methods=["GET", "POST"])
def webgl_loader_obj():

    obj_name = ''
    obj_name = request.args.get('obj_name')
    print('obj_name=', obj_name)
    return rt("webgl_loader_obj.html",obj_name=obj_name)
...
def custom_static(filename):
    ...
    return send_from_directory(
        "/Users/abel/AbelProject/JsRepository/B20017_threejs_list_detail/upload/",
        # "D:\\bysj\\user\\upload",
        filename,
    )
